...
const MemoryBlockRenderer: React.FC<BlockRendererProps> = ({
  blockId,
  blockType,
  blockVersion,
  data
  ...
  switch (blockType) {
    case 'project':
      ...
      titleContent = `Project: ${projectTitleFromMetadata || data.text || blockId}`;
      mainContent = <div ref={contentRef}>{data.metadata?.description ? String(data.metadata.description) : data.text}</div>;
      break;
    case 'doc':
      const docTitle = typeof data.metadata?.title === 'string' ? data.metadata.title : undefined;
      titleContent = docTitle || data.text.substring(0, 50) || blockId;
      mainContent = <div ref={contentRef}>{data.text}</div>;
      headerSuffix = (
        <div className="flex items-center space-x-1 text-xs">
          <span className="text-muted-foreground">Version:</span>
          <span className="font-medium">{blockVersion}</span>
        </div>
      );
      break;
    case 'task':
      titleContent = `Task: ${data.text.substring(0, 50)}...`;
      mainContent = <div ref={contentRef}>{data.text}</div>;
      break;
    case 'log':
      titleContent = `Log Entry: ${new Date(data.created_at || Date.now()).toLocaleString()}`;
      mainContent = <div ref={contentRef}>{data.text}</div>;
      break;
    default:
      titleContent = `Unknown Type: ${blockType} (${blockId})`;
      break;
  }
...
export default MemoryBlockRenderer;
